...
export class EstimatesController {
  ...
  findOne(@Param('id') id: string) {
    return this.estimatesService.findOne(Number(id));
  }

  @Patch(':id')
  update(@Param('id') id: string, @Body() updateEstimateDto: UpdateEstimateDto) {
    return this.estimatesService.update(Number(id), updateEstimateDto);
  }

  @Delete(':id')
  remove(@Param('id') id: string) {
    return this.estimatesService.remove(Number(id));
  }
}
